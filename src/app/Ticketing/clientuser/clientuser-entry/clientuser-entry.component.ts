...
import { ClientUserEntryService } from './clientuser-entry.service';

@Component({
  selector: 'app-clientuser-entry',
  templateUrl: './clientuser-entry.component.html',
  styleUrls: ['./clientuser-entry.component.scss']
})

export class ClientUserEntryComponent implements OnInit {
  ...
  constructor(
	  private dapiService: ClientUserEntryService,
      private _ui: UIService,
      private _msg: MessageBoxService,
      private _auth: AuthService,
      private _globals: AppGlobals,
      private _select: SelectService,
      private dialogRef: MatDialogRef<ClientUserEntryComponent>,
  ...
  ngOnInit() {
    if(localStorage.getItem(this._globals.baseAppName + '_language') == "16001") {
      ...
      this.dapiService.Controllers(this.pModel).subscribe(res => {
        ...
        this.data[2].editPrice = true
        if (this.data[2].value =="9") {
          this.data[3].editPrice = true
        }else {

          this.data[3].editPrice = false
        }
  ...
  onRoleChange(tableId: number,id: number, i: number) {
    console.log(this.light);
    
    if (tableId === 683) {
      if (id === 9) {
        this.light[1].editPrice = true
      }else {
        this.light[1].editPrice = false
      }
    }
  }
  onSubmit() {
    this.data[4].value = this.data[4].value.replace(/\s/g, '')
