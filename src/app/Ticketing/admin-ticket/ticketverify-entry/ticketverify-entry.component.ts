...
import { TicketVerifyEntryService } from './ticketverify-entry.service';

@Component({
  selector: 'app-ticketverify-entry',
  templateUrl: './ticketverify-entry.component.html',
  styleUrls: ['./ticketverify-entry.component.scss']
})

export class AdminTicketVerifyEntryComponent implements OnInit {
  ...
  constructor(
	  private dapiService: TicketVerifyEntryService,
      private _ui: UIService,
      private _msg: MessageBoxService,
      private _auth: AuthService,
      private _globals: AppGlobals,
      private _select: SelectService,
      private dialogRef: MatDialogRef<AdminTicketVerifyEntryComponent>,
  ...
  ngOnInit() {
    if(localStorage.getItem(this._globals.baseAppName + '_language') == "16001") {
      ...
      this.dapiService.Controllers(this.pModel.dataRe).subscribe(res => {
        this._ui.loadingStateChanged.next(false);
        this.data = res;
        this.dapiService.getClosedTicketId(this.pModel.tick.ticketId).subscribe(res => {
          // console.log(res);
          
          this.data[1].value = res[0].id.toString()
        })
        
        this.data[3].value = this._auth.getUserId()
