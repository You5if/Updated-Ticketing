...
import { AdminTicketEntryComponent } from './ticket-entry/ticket-entry.component';
...
import { AdminAssignedComponent } from './assigned-opt/checkfordelete.component';
import { AdminTransferComponent } from './transfer-opt/checkfordelete.component';
import { AdminTicketCloseEntryComponent } from './ticketclose-entry/ticketclose-entry.component';
import { AdminTicketVerifyEntryComponent } from './ticketverify-entry/ticketverify-entry.component';
import { GlobalSerivce } from 'src/app/global-functions.service';
import { AdminDetailsEntryComponent } from './details-entry/checkfordelete.component';
...
export class AdminTicketComponent implements OnInit {
  ...
  usersClick(selectedUserId:number) {
    // this.userId = selectedUserId
    this.ticketPageData.AppUserId = selectedUserId
  ...
  refreshMe() {
    this.badgeUnassigned = 0
    this.badgeAssigned = 0
    this.badgeClosed = 0
    this.badgeVerified = 0
    this.indexes = []
    this.indexesAssigned = []
    this.indexesClosed = []
    this.indexesVerify = []
    this.veiwIndexes = []
    this.veiwIndexesAssigned = []
    this.veiwIndexesClosed = []
    this.veiwIndexesVerify = []
...
creationSort(sortString: string) {
  ...
  openEntry  (result: TicketModel) {
    if (result === undefined) {
      const dialogRef = this.dialog.open(AdminTicketEntryComponent, {
        disableClose: true,
        data: {}
      });
  ...
  openDetails  (result: any) {
    if (result === undefined) {
      const dialogRef5 = this.dialog5.open(AdminDetailsEntryComponent, {
        disableClose: true,
  ...
  openEntry3  (result: Send, ticket: TicketModel) {
    if (result === undefined) {
      const dialogRef1 = this.dialog1.open(AdminTicketCloseEntryComponent, {
        disableClose: true,
  ...
  openEntry4 (result: Send, ticket: TicketModel) {
    if (result === undefined) {
      const dialogRef1 = this.dialog1.open(AdminTicketVerifyEntryComponent, {
        disableClose: true,
  ...
  openCommentDialog(result: TicketModel) {
    if (result === undefined) {
      const dialogRef2 = this.dialog2.open(AdminAddCommentComponent, {
        disableClose: true,
  ...
  openAssignedDialog (result: TicketModel) {
    if (result === undefined) {
      const dialogRef3 = this.dialog3.open(AdminAssignedComponent, {
        disableClose: true,
  ...
  openTransferDialog (result: TicketModel) {
    if (result === undefined) {
      const dialogRef4 = this.dialog4.open(AdminTransferComponent, {
        disableClose: true,
