...
import { EscalationEntryComponent } from './escalation-entry/escalation-entry.component';
...
export class EscalationComponent implements OnInit {
    ...
    clickedRows = new Set<EscalationModel>();
    selection = new SelectionModel<EscalationModel>(true, []);;
  ...
  onAdd  () {
    ...
    if(localStorage.getItem(this._globals.baseAppName + '_language') == "16001") {
      localStorage.setItem(this._globals.baseAppName + '_Add&Edit', "Add escalation");
    }else if(localStorage.getItem(this._globals.baseAppName + '_language') == "16002") {
      localStorage.setItem(this._globals.baseAppName + '_Add&Edit', "اضافة تصعيد");
    }
  ...
  onView = (id: number) => {
    ...
    this.escalationservice.getEscalationEntry(id).subscribe((result: EscalationModel) => {
      this._ui.loadingStateChanged.next(false);
      result.entryMode = 'V';
  ...
  onId(id: number, row:EscalationModel) {
  ...
  openEntry  (result: EscalationModel) {
    if (result === undefined) {
      const dialogRef = this.dialog.open(EscalationEntryComponent, {
        disableClose: true,
        data: {}
      });
