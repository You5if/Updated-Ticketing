...
export class SDCompanyService {
   ...
   getSDCompanyEntry(id: number): Observable<SDCompanyModel> {
      return this.httpClient.get<SDCompanyModel>(this._globals.baseAPIUrl + 'SDCompany/' + id).pipe(
      map((result: SDCompanyModel) => {
      return result;
   ...
   getSDCompanySubmit(data: SDCompanyModel) {
      data.auditColumns = {
      'userId': 1,
      'hostname': 'test',
      'ipaddress': 'test',
      ...
      switch (data.entryMode) {
          ...
          case 'E': {
          return this.http.post(this._globals.baseAPIUrl + 'SDCompany/edit', data, this._cf.requestOptions()).pipe(
          map((response: Response) => {
          return response.json();
          }), catchError(this._cf.handleError));
          }
          ...
          case 'D': {
          return this.http.post(this._globals.baseAPIUrl + 'SDCompany/delete', data, this._cf.requestOptions()).pipe(
          map((response: Response) => {
          return response.json();
          }), catchError(this._cf.handleError));
          }
